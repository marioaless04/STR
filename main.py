...
# Semáforo: Controla el flujo de trabajo entre hilos.
...
def sensor_acquisition():
    ...
    try:
        ...
        print("\nFinalizando hilo de adquisición de datos.")       
def actuador_brazo():
    """ Hilo independiente que representa el brazo mecánico """
    try:
        while True:
            # ESPERA: El brazo se bloquea aquí hasta que el semáforo sea > 0
            sem_brazo.acquire() 
        ...
        with mtx:
            print(f"Error en el hilo del brazo: {e}")
...
if __name__ == "__main__":
   # Creamos dos hilos para demostrar procesamiento paralelo y sincronización
    try:
        ...
        #Dameon=true para que el hilo del brazo se cierre automáticamente al finalizar el hilo principal (sensor)
        t_brazo = threading.Thread(target=actuador_brazo, daemon=True) # Daemon para que cierre al terminar el principal

        t_brazo.start()
        t_sensor.start()

        #Esperamos al hilo del sensor para finalizar la simulación, el hilo del brazo se cerrará automáticamente al ser daemon
        while t_sensor.is_alive():
